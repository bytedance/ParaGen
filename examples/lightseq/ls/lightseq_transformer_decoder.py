...
from paragen.modules.decoders import register_decoder, AbstractDecoder
...
from .lightseq_transformer_decoder_layer import LSTransformerDecoderLayer


@register_decoder
class LSTransformerDecoder(AbstractDecoder):
    """
    ...
    def __init__(self,
        ...
        self._activation_dropout = activation_dropout
    ...
    def build(self,
        ...
        env = Environment()
        if 'train' in env.configs['dataloader']:
            conf = env.configs['dataloader']['train']
        else:
            conf = [_ for _ in env.configs['dataloader'].values()][0]
        max_tokens = search_key(conf, 'max_tokens')
        maxlen = search_key(env.configs, 'maxlen')
        ...
        self._layers = nn.ModuleList([LSTransformerDecoderLayer(d_model=self._d_model,
                                                                nhead=self._n_head,
                                                                dim_feedforward=self._dim_feedforward,
                                                                dropout=self._dropout,
                                                                activation=self._activation,
                                                                attention_dropout=self._attention_dropout,
                                                                activation_dropout=self._activation_dropout,
