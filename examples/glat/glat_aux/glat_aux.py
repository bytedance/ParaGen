...
@register_model
class GLATAuxModel(AbstractEncoderDecoderModel):
    """
    GLATAuxModel is glancing transformer with an auxiliary decoder.
    ...
    def __init__(self,
        ...
        self._aux_decoder_config = aux_decoder
    ...
    def _build(self, src_vocab_size, tgt_vocab_size, src_special_tokens, tgt_special_tokens):
        ...
        self._aux_decoder = create_decoder(self._aux_decoder_config)
        ...
        self._aux_decoder.build(embed=tgt_embed,
                                special_tokens=tgt_special_tokens,
                                out_proj=tgt_out_proj)
    ...
    def forward(self,
        ...
        decoder_embed, orig_decoder_embed = self.calc_decoder_input(src_padding_mask,
                                                tgt_padding_mask,
                                                target,
                                                fusing_target_mask)
        with local_seed(self.seed):
            ...
            aux_logits = self._aux_decoder(tgt=orig_decoder_embed,
                                   memory=src_hidden,
                                   tgt_padding_mask=tgt_padding_mask,
                                   memory_padding_mask=src_padding_mask)
        return {'token': logits, 'length': length_logits, 'aux_token': aux_logits}
    ...
    def calc_decoder_input(self,
        ...
        if self._decoder_input == 'unk':
            decoder_input = create_sequence(tgt_padding_mask, self._tgt_special_tokens['unk'])
