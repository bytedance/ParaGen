...
@register_task
class CTCTranslationTask(TranslationTask):
    """
    ...
    def _build_criterions(self):
        ...
        self._criterion.build(self._model, padding_idx=self._tokenizer[self._tgt].pad,blank_idx=self._tokenizer[self._tgt].unk)
    ...
    def _collate(self, samples: List[Dict]) -> Dict[str, List[Tensor]]:
        ...
        src_padding_mask = src.eq(self._tokenizer[self._src].pad)
        output_padding_mask = src_padding_mask.unsqueeze(-1).repeat(1,1,2).view(src_padding_mask.size(0),-1)
        if not self._infering:
            ...
            batch = {
                'net_input': {
                    'src': src,
                    'tgt_padding_mask': output_padding_mask,
                },
                'net_output': {
                    'target': tgt,
            ...
            net_input = {'src': src, 'tgt_padding_mask': output_padding_mask}
